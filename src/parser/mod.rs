...
mod tests {
    ...
    #[test]
    fn sanitizer_removes_active_content_and_event_handlers() {
        let sanitized = sanitize_content(
            r#"<p>Safe text</p><img src="x" onerror="alert(1)"><script>alert(2)</script>"#,
        );

        assert!(sanitized.contains("Safe text"));
        assert!(!sanitized.contains("onerror"));
        assert!(!sanitized.contains("<script"));
        assert!(!sanitized.contains("alert(2)"));
    }

    #[test]
    fn sanitizer_rejects_mathml_annotation_integration_markup() {
        let sanitized = sanitize_content(
            r#"<math><annotation-xml encoding="text/html"><style><img src=x onerror=alert(1)></style></annotation-xml></math>"#,
        );

        assert!(!sanitized.contains("annotation-xml"));
        assert!(!sanitized.contains("onerror"));
